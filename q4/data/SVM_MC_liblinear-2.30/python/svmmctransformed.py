...
ftrain = open('train_transform.txt')
rxid,rxletter,rxnextid,rxwordid,rxpixels,rxposition = [],[],[],[],[],[]
for line in ftrain:
    fields=line.split(" ")
    fields[-1] = fields[-1].strip()
    rxid.append(fields[0])
    rxletter.append(lindex[fields[1]])
    rxnextid.append(fields[2])
    rxwordid.append(fields[3])
    rxposition.append(fields[4])
    temp=[]
    for i in range(len(fields[5:])):
        temp.append(fields[5+i])
    rxpixels.append(temp)
ftrain.close()
...
tranval = [0,500,1000,1500,2000]
for tval in tranval:
        print('Transformation case: ',tval)
        with open('dftrain.txt', 'w') as filehandle:     
                for li in range(len(xletter)):
                        if li>=tval:
                                ...
                                filehandle.write('%s ' % rxletter[li])
                                for i in range(len(rxpixels[li])):
                                    filehandle.write('%s:' % (i+1) )
                                    filehandle.write('%s ' %rxpixels[li][i] )
                                filehandle.write('\n')
